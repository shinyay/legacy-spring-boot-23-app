...
const InventoryRotationMatrix = ({ inventoryData, showChart = true }) => {
  ...
  if (!inventoryData || inventoryData.length === 0) {
    return (
      <Card className={classes.root}>
        <CardContent>
          <Typography variant="h6">在庫回転マトリックス</Typography>
  ...
  const quadrantConfig = {
    STAR: {
      title: 'スター商品',
      color: '#4caf50',
      description: '高回転・新しい売上',
      backgroundColor: '#e8f5e8',
    },
    QUESTION: {
      title: 'クエスチョン商品',
      color: '#ff9800',
      description: '低回転・新しい売上',
      backgroundColor: '#fff3e0',
    },
    CASH_COW: {
      title: 'キャッシュカウ商品',
      color: '#2196f3',
      description: '高回転・古い売上',
      backgroundColor: '#e3f2fd',
    },
    DOG: {
      title: 'ドッグ商品',
      color: '#f44336',
      description: '低回転・古い売上',
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <Paper style={{ padding: 8 }}>
          ...
          <Typography variant="body2">回転率: {data.turnoverRate?.toFixed(1)}</Typography>
          <Typography variant="body2">最終販売: {data.daysSinceLastSale}日前</Typography>
          <Typography variant="body2">在庫: {data.currentStock}冊</Typography>
          <Typography variant="body2">分類: {quadrantConfig[categorizeItem(data)]?.title}</Typography>
        </Paper>
      );
    }
    return null;
